...
export const payMpesaOrder = (amount) => async (dispatch, getState) => {
  try {
    dispatch({
      type: ORDER_MPESA_REQUEST,
    });
    const {
      userLogin: { userInfo },
    } = getState();
    ...
    const { data } = await axios.post(`/api/orders/mpesa`, amount, config);
    dispatch({
      type: ORDER_MPESA_SUCCESS,
      payload: data,
    });
  } catch (error) {
    dispatch({
      type: ORDER_MPESA_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
